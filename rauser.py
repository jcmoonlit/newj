...
class User_Agent:
    ...
    def get_list(version):
        ...
        # UC浏览器
        userAgent_phone_list.append('UCWEB7.0.2.37/28/999')

        userAgent_phone_list.append('NOKIA5700/ UCWEB7.0.2.37/28/999')

        # UCOpenwave
        userAgent_phone_list.append('penwave/ UCWEB7.0.2.37/28/999')

        # UC Opera
        userAgent_phone_list.append('Mozilla/4.0 (compatible; MSIE 6.0; ) Opera/UCWEB7.0.2.37/28/999')

        if version == 'pc':
            return userAgent_list
        if version == 'phone':
            return userAgent_phone_list

    @staticmethod
    def get_user_agent(version):
        list = []
        if version == 'pc':
            list = User_Agent.get_list('pc')
            userAgent_dic = {'User-Agent': random.choice(list)}
            return userAgent_dic

        if version == 'phone':
            list = User_Agent.get_list('phone')
            userAgent_dic = {'User-Agent': random.choice(list)}
            return userAgent_dic


if __name__ == '__main__':
    user = User_Agent.get_user_agent(version='pc')
    print(user)